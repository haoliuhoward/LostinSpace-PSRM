...
base.path<-"/home/vis/jl324/R/LearningLocations"
...
# load data
drc_data<-read.csv("drc_cleantext.csv") #text files pre-treated. (output from text_treatment_drc.R)
...
# and the province coded by ICEWS
...
set.seed(2015)
sampled1<-sample(1:3, nrow(drc_data), replace=TRUE)
set.seed(2016)
sampled2<-sample(1:3, nrow(drc_data), replace=TRUE)
set.seed(2017)
sampled3<-sample(1:3, nrow(drc_data), replace=TRUE)
...
for(i in 1:k){
  ...
  set2<-drc_data[sampled==t,]
  set1<-drc_data[sampled!=t,]
  
  
  #### Step 2-a: build corpus -- set1 (train set) ####
  data<-set1 
  ...
  #### Step 2-b: feature selection -- set1 (still the same train set) ####
  data<-set1
  ...
  #### Step 2-c: feature selection -- set2 test set ####
  data<-set2
  # build the Dependent variable based on human coding (1: correct event location, 0: otherwise)
  binary_data<-buildY(texts=data$cleantext,  
                      namespace=names,
...
drc.results.path<-paste0(drc.path, "/results")
setwd(drc.results.path)
results_drc<-as.data.frame(cbind(rf,svm,nnet,logit))
write.csv(results_drc, "results_drc.csv")
...
drc_all<-buildY(texts=drc_data$cleantext, namespace=names,
                solution=drc_data$province_human, text_no=drc_data$story_id)
drc_all$id<-paste0(drc_all$text_id, drc_all$location_names)
...
my.sum<-function(input){
  ...
  # /sum(str_count(input, '0|1'), na.rm=TRUE)
}
...
for( i in 1:length(id)){
  y[i]<-as.numeric(as.character(drc_all$Y[which(id[i]==drc_all$id)] ) )
}

drc_results.all<-as.data.frame(cbind(id, "original"=y,
...
write.csv(drc_results.all, "drc_results.all.csv")
